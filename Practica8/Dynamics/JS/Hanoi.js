...
//De otro modo se generan las cookies
else {
  pos1 = 1;
  pos2 = 1;
  pos3 = 1;
...
//Se  genera el tiempo que el jugador ha tardado en ganar, así como las cookies
function ganar() {
  let fin = new Date();
...
//Se buscan las cookies de las posiciones
function buscarCookie(pos) {
  ...
  //En caso de que la posicion sea igual a 2  se hara el if
  if (pos == 2) {
    let regex = /pos2=[1-3]/;
    let indice = cookies.search(regex);
    if (indice != -1) {
    ...
    //En caso de que la posicion sea igual a 3 se hara el if
    if (pos == 3) {
      let regex = /pos3=[1-3]/;
      let indice = cookies.search(regex);
      if (indice != -1) {
    ...
    //En caso de que la posicion sea igual a 4 se hara el if
    if (pos == 4) {
      let regex = /pos4=[1-3]/;
      let indice = cookies.search(regex);
      if (indice != -1) {
    ...
    //En caso de que la posicion sea igual a 5 se hara el if
    if (pos == 5) {
      let regex = /pos5=[1-3]/;
      let indice = cookies.search(regex);
      if (indice != -1) {
...
//Se valida que el disco que se ponga sobre otro no sea de mayor tamaño
...
//valida que la torre de llegada exista
...
//se actualiza la posicion de las fichas
